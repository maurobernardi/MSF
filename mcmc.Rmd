...
The R markdown source for this document is
https://www.stat.umn.edu/geyer/3701/notes/mcmc-bayes.Rmd.
...
books and courses like STAT 5101â€“5102 do not explicitly discuss it
...
$X$ and $Y$ or of the random vector $(X, Y)$.
The only point of the words is to indicate that a "marginal" distribution
is for a subset of variables of the "joint distribution".
...
This process of "finding the 'other' conditional" â€” we are given
one conditional $f(x \mid \theta)$ and have to find the other conditional
$f(\theta \mid x)$, which is the posterior â€” is often called
...
(anything you call a confidence interval is one â€” it has *some*
...
In our example application below, we [analyze a question that frequentists
deny is meaningful](#output-function).
...
[TL;DR](https://www.google.com/search?q=tldr)
...
the log unnormalized posterior (log likelihood + log unnormalized prior),
then you can do Bayesian inference.
...
It is a [Goldilocks problem](https://en.wikipedia.org/wiki/Goldilocks_and_the_Three_Bears) â€” so-called because of [one bowl of porridge being too hot,
...
We notice three issues.
 ...
 #. `Coefficients: (1 not defined because of singularities)`.  Yes. that
    ...
    $0 / 0$ which is undefined, and the probability of being greater
    than something undefined, which is again undefined.
...
(There are [Stat 5421 notes on this subject](https://www.stat.umn.edu/geyer/5421/notes/infinity.html).)
...
code chunks on which we depend need names (also called labels).
...
See the [section below on debugging a buggy
log unnormalized density function](#debug-try-1) for a bad time series plot.
...
Chain Monte Carlo, Chapter 11](http://www.mcmchandbook.net/HandbookSampleChapters.html) would
...
are just parameters ([a rose by any other name would smell as sweet](https://en.wikipedia.org/wiki/A_rose_by_any_other_name_would_smell_as_sweet)).
...
completely a theoretical artifact â€” not a parameter of the distribution
of the data.
...
in a hierarchical Bayesian model â€” they may *think* they understand, but they're wrong â€”
